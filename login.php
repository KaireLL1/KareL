...
<body style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;">
  ...
  <div style="width:100%;max-width:360px;position:relative;">
    ...
    <div style="text-align:center;margin-bottom:44px;">
      ...
      <p style="color:#555;font-size:14px;margin-top:4px;">Share momenmu dengan teman</p>
    </div>

    <?php if ($error): ?>
    <div id="login-error" style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);color:#f87171;border-radius:14px;padding:12px 16px;margin-bottom:16px;font-size:14px;">
      <?= htmlspecialchars($error) ?>
    </div>
    <?php endif; ?>

    <form method="POST" id="login-form" style="display:flex;flex-direction:column;gap:12px;">
      <input id="email-input" class="k-input" type="email" name="email" placeholder="Email" required
        value="<?= htmlspecialchars($_POST['email'] ?? '') ?>">
      <input id="password-input" class="k-input" type="password" name="password" placeholder="Password" required>
      <div style="margin-top:4px;">
        <button id="login-btn" class="k-btn" type="submit">Masuk</button>
      </div>
    </form>

    <p style="text-align:center;color:#444;font-size:14px;margin-top:24px;">
      Belum punya akun?
      <a id="to-register" href="register.php" style="color:#FFD60A;font-weight:700;text-decoration:none;"> Daftar</a>
    </p>
  </div>
